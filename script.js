...
const toggleProductsVisibility = () => {
  ...
  const productsOffset = products.getBoundingClientRect().top;
  ...
  if (st > lastScrollTop) {
    ...
    h1Element.classList.remove("show");
  }
  lastScrollTop = st <= 0 ? 0 : st;

  if (productsOffset <= windowHeight / 2) {
...
let lastScrollTop = 0;
window.addEventListener("scroll", toggleProductsVisibility);
...
document
  .querySelector(".finalize-cart-button")
  .addEventListener("click", function () {
    ...
    const cartMessage = `I'd like to order the following items:\n${cart
      ...
      navigator.permissions
        ...
        .then((result) => {
          if (result.state === "granted" || result.state === "prompt") {
            navigator.clipboard
              .writeText(cartMessage)
              .then(function () {
                alert(
                  "Order details copied. Please paste this in our Instagram DM to complete your order."
                );
                redirectToInstagram();
              })
              .catch(function (err) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  copyBtn.addEventListener("click", function () {
    var cartMessage = document.getElementById("cartMessage").textContent;

    var textArea = document.createElement("textarea");
    textArea.value = cartMessage;
...
function redirectToInstagram() {
  const instagramURL = `https://ig.me/m/anvat.nyc`;
